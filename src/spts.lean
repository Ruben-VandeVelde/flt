...
begin
  ...
  { obtain ⟨c, d, hcd⟩ := factors2 H,
    ...
    { rw hg',
      apply nat.lt_mul_of_one_lt_left hg'pos,
      norm_num },
  ...
  { by_cases g = 1,
    ...
    { rw ←ne.def at h,
      ...
      obtain ⟨q, hq⟩ := pdvd,
      have hqpos : 0 < q,
      { apply pos_of_mul_pos_left _ (zero_le p),
        rwa ←hq },
      refine IH q _ hqpos _ c d _,
      { rw [hq, hAB],
        apply nat.lt_mul_of_one_lt_left _ pprime.one_lt,
        apply pos_of_mul_pos_left _ (zero_le p),
        rwa ←hq },
      { intros f' hf'dvd hf'odd,
        refine hnotform f' _ hf'odd,
      ...
      { rw [←nat.mul_right_inj pprime.pos, hcd, ←hfactor, hq, ←hAB, mul_left_comm] } } }
end
