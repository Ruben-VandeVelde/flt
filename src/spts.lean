...
lemma factors'
  ...
  (hodd : ¬even f)
  (hgpos : 0 < g)
  (hfactor : (abs (f * g) : ℤ) = a.norm)
  (hnotform : ∀ (f' : ℤ),
                 f' ∣ g →
                ¬even f' → (∃ (p : ℤ√-3), (abs f' : ℤ) = p.norm)) :
  ∃ (p : ℤ√-3), (abs f : ℤ) = p.norm :=
begin
  ...
  { obtain rfl|h := (nat.succ_le_of_lt hgpos).eq_or_lt,
    ...
    { obtain ⟨p, pprime, pdvd⟩ := nat.exists_prime_and_dvd (nat.succ_le_of_lt h),
      have : (p : ℤ) ∣ a.norm,
      { rw [←hfactor, dvd_abs],
        exact dvd_mul_of_dvd_right (int.coe_nat_dvd.mpr pdvd) _ },
...
begin
  revert x a,
  intro x',
  apply int.strong_induction_on x',
  clear x',
  intros x IH a hcoprime hodd hfactor,
  have hneg1 : 1 ≤ a.norm,
  { rw [←int.sub_one_lt_iff, sub_self],
    apply lt_of_le_of_ne (zsqrtd.norm_nonneg _ a),
    symmetry,
    intro H,
    rw zsqrtd.norm_eq_zero_iff _ at H,
  ...
  obtain ⟨m, c, ha, hc⟩ := int.factor_div a.re x hodd h0,
  obtain ⟨n, d, hb, hd⟩ := int.factor_div a.im x hodd h0,
  set c' : ℤ√-3 := ⟨c, d⟩ with hc',
  have h2 : a = c' + ⟨m, n⟩ * x,
  ...
  { contrapose! h with H,
    apply le_of_eq,
    simp only [zsqrtd.ext, zsqrtd.zero_re, zsqrtd.zero_im, hc'] at H,
    simp only [H.1, H.2, zero_add] at ha hb,
    rw ←int.is_unit_iff_abs,
    apply hcoprime.is_unit_of_dvd' _ _;
    { simp only [←ha, ←hb],
  ...
  { set e : ℤ := m ^ 2 * x + 2 * m * c + 3 * n ^ 2 * x + 6 * n * d with he,
    ...
    rw [he, zsqrtd.norm_def, zsqrtd.norm_def, ←ha, ←hb],
    ring },

  have h3 : y.nat_abs < x.nat_abs,
  { rw [←mul_lt_mul_left h0'', ←pow_two],
    rw ←mul_lt_mul_left (by norm_num : 0 < 4),
    rw [←int.nat_abs_mul, ←hy],
    zify,
    rw int.nat_abs_pow_two,
    rw int.nat_abs_of_nonneg (zsqrtd.norm_nonneg (by norm_num) c'),
  ...
  { rw [hg, pos_iff_ne_zero, int.gcd_ne_zero_iff],
    contrapose! h1 with H,
    simp only [zsqrtd.ext, H, zsqrtd.zero_im, and_self, zsqrtd.zero_re] },
  have hgnezero := int.coe_nat_ne_zero_iff_pos.mpr hgpos,
  obtain ⟨C', HC'⟩ : ((g : ℤ) : ℤ√-3) ∣ c',
  { rw zsqrtd.coe_int_dvd_iff,
    exact ⟨int.gcd_dvd_left _ _, int.gcd_dvd_right _ _⟩ },
  have HCDcoprime : is_coprime C'.re C'.im,
  { simp only [zsqrtd.ext, int.cast_coe_nat, add_zero, zero_mul, zsqrtd.coe_nat_re, zsqrtd.mul_im,
      zsqrtd.mul_re, mul_zero, zsqrtd.coe_nat_im] at HC',
    rw ←int.div_eq_of_eq_mul_right hgnezero HC'.1,
    rw ←int.div_eq_of_eq_mul_right hgnezero HC'.2,
    exact int.coprime_div_gcd_div_gcd hgpos },
  ...
  { apply int.eq_of_mul_eq_mul_left (pow_ne_zero 2 hgnezero),
    rw [←h5, hz],
    ring },
  have h6' : z ∣ C'.norm := h6 ▸ dvd_mul_left z x,
  ...
  refine IH w _ C' HCDcoprime hwodd _,
  { calc w.nat_abs
        ≤ z.nat_abs : nat.le_of_dvd (int.nat_abs_pos_of_ne_zero h8) (int.nat_abs_dvd_abs_iff.mpr hwdvd)
