...
begin
  ...
  obtain h|⟨hp, hodd⟩ := h,
  { rw [H, is_coprime_zero_right, int.is_unit_iff_abs] at hcoprime,
...
begin
  ...
  rw int.nat_abs_of_nonneg (zsqrtd.norm_nonneg (by norm_num) z),
...
begin
  ...
  { obtain ⟨c, hc⟩ := factors a x hcoprime hodd hfactor,
    ...
    intro H,
    apply pow_not_prime one_lt_two,
    convert hprime.abs,
...
begin
  suffices : ∀ n : ℕ, a.norm = n →
    ...
    ∀ pq : ℤ√-3, pq ∈ f →
      ...
      pq.im ≠ 0 ∧
      odd_prime_or_four pq.norm,
  { exact this a.norm.nat_abs (int.nat_abs_of_nonneg (zsqrtd.norm_nonneg (by norm_num) a)).symm },

  intros n hn,
  induction n using nat.strong_induction_on with n ih generalizing a,
  dsimp only at ih,
  have hn' : a.norm.nat_abs = n,
  { rw hn, exact int.nat_abs_of_nat n },

  by_cases h : a.norm = 1,
  { have : abs a.re = 1 ∧ a.im = 0,
    { exact spts.eq_one h },
    use 0,
    split,
  ...
  { have : 1 < a.norm,
    ...
    have : 2 < a.norm,
    { rw [←int.add_one_le_iff, ←mul_two, one_mul] at this,
    ...
    rw [hn'] at descent,
    obtain ⟨g, hgprod, hgfactors⟩ := ih U.norm.nat_abs descent huvcoprime
      (int.nat_abs_of_nonneg (zsqrtd.norm_nonneg (by norm_num) _)).symm,
    refine ⟨q ::ₘ g, _, _⟩,
    { rw huv,
...
begin
  suffices : ∀ n : ℕ, (n : ℤ) = a.norm → even (even_factor_exp n),
  { have h0 := zsqrtd.norm_nonneg (by norm_num) a,
    have h1 := (int.nat_abs_of_nonneg h0),
    rw ←h1,
    exact this a.norm.nat_abs h1 },
  intros n hn,
  induction n using nat.strong_induction_on with n ih generalizing a,
  dsimp only at ih,
  ...
  { obtain ⟨u, huvcoprime, -, huvprod⟩ := step1' hcoprime hparity,
    ...
    set m := u.norm.nat_abs with hm,
    have hm' : (m : ℤ) = u.norm := int.nat_abs_of_nonneg huv.le,
    rw [hn, huvprod, factors_2_even huv.ne', nat.even_add, ←hm'],
    apply iff_of_true (dvd_refl _),
    apply ih _ _ huvcoprime hm',
    zify,
    rw [hm', hn, huvprod, lt_mul_iff_one_lt_left huv],
    norm_num },
...
begin
  ...
  { have := unique_factorization_monoid.factors_prod (spts.ne_zero_of_coprime' _ hcoprime),
...
begin
  ...
  rw [←multiset.count_nsmul x.norm, ←factors_odd_prime_or_four.pow, hcube],
  rw multiset.countp_eq _ (λ y : ℤ√-3, x.norm = y.norm),
  rw ←multiset.count_map x.norm,
  congr',
